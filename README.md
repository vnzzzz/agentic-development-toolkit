...
親workspace自身が利用する汎用Skillは`vnzzzz/agent-skills` Pluginとして導入します。
...
├── scripts/                    # Skill探索・検証・Plugin導入、親監査
...
├── .claude/skills/             # 開発中local Skill用の生成リンク
└── .agents/skills/             # 開発中local Skill用の生成リンク
```
...
`.claude/skills/<name>`と`.agents/skills/<name>`は、repository形式に関係なく開発中のlocal Skill rootへ直接linkします。
...
2. post-createがClaude Code / Codex CLIと`agent-skills` Pluginを導入する。
3. Claude Codeは`claude`、Codexは`codex`で必要な認証を行う。
4. `make doctor`で環境を確認する。
5. `make test`で親workspaceを検証する。

`agent-skills`はpublic GitHub repositoryのHTTPS Git URLからuser scopeへ導入します。
GitHub tokenやSSH鍵は要求せず、GitHubへの外向きHTTPS通信を前提とします。
特定revisionへpinせず、Dev Container作成時にmarketplaceを更新して最新Pluginを利用します。

post-createはAgent CLI、共有Plugin、local Skillへのlinkを用意します。Skill固有依存やSkill固有testは自動実行せず、各source repositoryの手順に従います。

Dev Containerを使わない場合は`make bootstrap`で親用のローカルPython環境と探索linkを用意できます。Agent CLIと共有Plugin、source repository固有依存は導入しません。
...
共有Skillの通常利用はPlugin、Skill本体を変更するときだけ`repos/`のmutable working copyを使います。
...
共有Pluginの導入処理だけを手動確認する場合は、Agent CLI導入後に次を実行します。
...
- 親CI: 親のPython・shell・設定、探索/検証/linkロジック、Plugin bootstrapのCodex / Claude Code実CLI integration、standalone template、親セキュリティ設定。
